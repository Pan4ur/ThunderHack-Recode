...
public class Core {
    ...
    public void onSync(EventSync event) {
        if (fullNullCheck())
            return;
        Thunderhack.moduleManager.get(thunder.hack.modules.movement.Timer.class).onEntitySync(event);
    }
    ...
    public void drawGps(Render2DEvent e){
        if (Thunderhack.gps_position != null) {
            float xOffset = Util.getScaledResolution().getScaledWidth() / 2f;
            float yOffset = Util.getScaledResolution().getScaledHeight() / 2f;
            ...
            e.getMatrixStack().translate(-xOffset, -yOffset, 0.0F);
            Thunderhack.moduleManager.get(RadarRewrite.class).drawTracerPointer(e.getMatrixStack(),xOffset, yOffset - 50, 12.5f, ClickGui.getInstance().getColor(1).getRGB());
            e.getMatrixStack().translate(xOffset, yOffset, 0.0F);
            e.getMatrixStack().multiply(RotationAxis.POSITIVE_Z.rotationDegrees(-yaw));
            e.getMatrixStack().translate(-xOffset, -yOffset, 0.0F);
            RenderSystem.setShaderColor(1F, 1F, 1F, 1F);
    ...
    public void drawLagNotify(Render2DEvent e){
        ...
        if(!rubberbandTimer.passedMs(5000)){
            ...
            if(MainSettings.language.getValue() == MainSettings.Language.RU) {
                FontRenderers.modules.drawCenteredString(e.getMatrixStack(), "Обнаружен руббербенд! " + decimalFormat.format((5000f - (float) rubberbandTimer.getTimeMs()) / 1000f), (float) Util.getScaledResolution().getScaledWidth() / 2f, (float) Util.getScaledResolution().getScaledHeight() / 3f, new Color(0xFFDF00).getRGB());
            } else {
                FontRenderers.modules.drawCenteredString(e.getMatrixStack(), "Rubberband detected! " + decimalFormat.format((5000f - (float) rubberbandTimer.getTimeMs()) / 1000f), (float) Util.getScaledResolution().getScaledWidth() / 2f, (float) Util.getScaledResolution().getScaledHeight() / 3f, new Color(0xFFDF00).getRGB());
            }
        }
        if(packetTimer.passedMs(5000)){
            DecimalFormat decimalFormat = new DecimalFormat( "#.#" );
            if(MainSettings.language.getValue() == MainSettings.Language.RU) {
                FontRenderers.modules.drawCenteredString(e.getMatrixStack(), "Сервер перестал отвечать! " + decimalFormat.format((float) packetTimer.getTimeMs() / 1000f), (float) Util.getScaledResolution().getScaledWidth() / 2f, (float) Util.getScaledResolution().getScaledHeight() / 3f, new Color(0xFFDF00).getRGB());
            } else {
