...
public class AntiAim extends Module {
    ...
    public void onSync(EventSync e) {
        ...
        if (yawMode.getValue() != Mode.None) {
            ...
            if (bodySync.getValue()) mc.player.setBodyYaw(rotationYaw);
        }
    ...
    public void onCalc(PlayerUpdateEvent e) {
        if (pitchMode.getValue() == Mode.RandomAngle)
            if (mc.player.age % Speed.getValue() == 0) rotationPitch = MathUtility.random(90, -90);

        if (yawMode.getValue() == Mode.RandomAngle)
            if (mc.player.age % Speed.getValue() == 0) rotationYaw = MathUtility.random(0, 360);

        if (yawMode.getValue() == Mode.Spin) if (mc.player.age % Speed.getValue() == 0) {
            rotationYaw += yawDelta.getValue();
            if (rotationYaw > 360) rotationYaw = 0;
            if (rotationYaw < 0) rotationYaw = 360;
        }

        if (pitchMode.getValue() == Mode.Spin) if (mc.player.age % Speed.getValue() == 0) {
        ...
        if (pitchMode.getValue() == Mode.Fixed) rotationPitch = pitchDelta.getValue();

        if (yawMode.getValue() == Mode.Fixed) rotationYaw = yawDelta.getValue();

        if (pitchMode.getValue() == Mode.Static) {
            rotationPitch = mc.player.getPitch() + pitchDelta.getValue();
            rotationPitch = MathUtility.clamp(rotationPitch, -90, 90);
        }
        if (yawMode.getValue() == Mode.Static) rotationYaw = mc.player.getYaw() % 360 + yawDelta.getValue();
    ...
    public enum Mode {
        None, RandomAngle, Spin, Sinus, Fixed, Static, Jitter
    }
}