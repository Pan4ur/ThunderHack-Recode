...
public class Trails extends Module {
    ...
    public class Particle {
        ...
        public void render(MatrixStack matrixStack, BufferBuilder bufferBuilder) {
            ...
            bufferBuilder.vertex(matrix, 0, -scale, 0).texture(0, 1).color(applyOpacity(c, 1f - colorAnim).getRGB());
            bufferBuilder.vertex(matrix, -scale, -scale, 0).texture(1, 1).color(applyOpacity(c, 1f - colorAnim).getRGB());
            bufferBuilder.vertex(matrix, -scale, 0, 0).texture(1, 0).color(applyOpacity(c, 1f - colorAnim).getRGB());
            bufferBuilder.vertex(matrix, 0, 0, 0).texture(0, 0).color(applyOpacity(c, 1f - colorAnim).getRGB());
            matrixStack.pop();
        }
