...
public class HitBubbles extends Module {
    ...
    public void onHit(EventAttack e) {
        Vec3d point = ThunderHack.playerManager.getRtxPoint(((IClientPlayerEntity) mc.player).getLastYaw(), ((IClientPlayerEntity) mc.player).getLastPitch(), 4);
        if (point != null) bubbles.add(new HitBubble((float) point.x, (float) point.y, (float) point.z, -((IClientPlayerEntity) mc.player).getLastYaw(), ((IClientPlayerEntity) mc.player).getLastPitch(), new Timer()));
    }
