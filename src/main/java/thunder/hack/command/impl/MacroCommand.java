...
public class MacroCommand extends Command {
    ...
    public void executeBuild(LiteralArgumentBuilder<CommandSource> builder) {
        ...
        builder.then(literal("remove").then(arg("name", StringArgumentType.word())).executes(context -> {
            ...
            if (Thunderhack.macroManager.getMacroByName(name) != null) {
                Macro macro = Thunderhack.macroManager.getMacroByName(name);
                Thunderhack.macroManager.removeMacro(macro);
                sendMessage("Удален макрос " + macro.getName());
            } else {
                sendMessage("Не существует макроса с именем " + name);
            }
