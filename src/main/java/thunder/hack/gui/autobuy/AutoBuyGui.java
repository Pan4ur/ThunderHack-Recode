...
public class AutoBuyGui extends Screen {
    ...
    public int main_posX = 100, main_posY = 100, prevCategoryY, CategoryY, slider_y, slider_x, main_height = 250, drag_x = 0, drag_y = 0, rescale_y = 0;
    ...
    private boolean dragging, rescale, first_open = true, searching = false;
    ...
    public void load() {
        ...
        if (current_category == Category.Items) {
            components.add(new ItemPlate(new AutoBuyItem(Items.AIR, new ArrayList<>(), -1, -1), main_posX + 100, main_posY + 40 + module_y, (int) (module_y / 35f)));
            module_y += 35;
    ...
    public void renderGui(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        FontRenderers.thglitch.drawString(context.getMatrices(), "AUTOBUY", 15, 0, ThunderHackGui.getColorByTheme(2).getRGB());
        context.getMatrices().translate(-(main_posX + 10) / 0.85, -(main_posY + 15) / 0.85, 0);
        ...
        FontRenderers.sf_medium.drawCenteredString(context.getMatrices(), "Очистить", main_posX + 50, main_posY + 168, -1);
        ...
        if (selected_plate != prev_selected_plate && selected_plate != null) {
            ...
            if (selected_plate.getAutoBuyItem().getItem() == Items.AIR) {
                AutoBuy.items.add(new AutoBuyItem(Items.DIRT, new ArrayList<>(), 100, 1));
                selected_plate = null;
                load();
            } else {
                settings.add(new AutoBuyValue("Item", selected_plate.getAutoBuyItem()));
                settings.add(new AutoBuyValue("Price", selected_plate.getAutoBuyItem()));
                settings.add(new AutoBuyValue("Enchants", selected_plate.getAutoBuyItem()));
                settings.add(new AutoBuyValue("Count", selected_plate.getAutoBuyItem()));
            }
        }
        ...
        FontRenderers.sf_medium.drawString(context.getMatrices(), "Предметов: " + (AutoBuy.items.size()), main_posX + 105, main_posY + 18, new Color(0x7BFFFFFF, true).getRGB(), false);
        ...
        if (!settings.isEmpty()) {
            ...
            FontRenderers.sf_medium_mini.drawString(context.getMatrices(), "Пример зачарований: protection:4 mending:0 unbreaking:2\nЧтобы удалить предмет, нажми по нему колесиком", main_posX + 210, main_posY + 150, new Color(0x7BFFFFFF, true).getRGB(), false);
        }
        Render2DEngine.popWindow();
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int clickedButton) {
        ...
        if (isHoveringItem(main_posX + 5, main_posY + 100, 90, 25, (float) mouseX, (float) mouseY)) {
            for (Block block : Registries.BLOCK)
                if (Item.fromBlock(block) != Items.AIR)
                    AutoBuy.items.add(new AutoBuyItem(Item.fromBlock(block), new ArrayList<>(), 10, 1));

            for (Item item : Registries.ITEM)
                if (item != Items.AIR)
                    AutoBuy.items.add(new AutoBuyItem(item, new ArrayList<>(), 10, 1));
            load();
        }
        ...
        if (isHoveringItem(main_posX + 5, main_posY + 130, 90, 25, (float) mouseX, (float) mouseY)) {
            for (Block block : Registries.BLOCK)
                if (Item.fromBlock(block) != Items.AIR)
                    AutoBuy.items.add(new AutoBuyItem(Item.fromBlock(block), new ArrayList<>(), 100, 1));

            for (Item item : Registries.ITEM)
                if (item != Items.AIR)
                    AutoBuy.items.add(new AutoBuyItem(item, new ArrayList<>(), 100, 1));
            load();
        }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        ...
        components.forEach(component -> component.scrollElement(dWheel * 2));
        if (current_category == Category.Log)
            logs.forEach(log -> log.scrollElement(dWheel * 2));
