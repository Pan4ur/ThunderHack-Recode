...
public class ThunderGui extends Screen {
    ...
    public void load() {
        ...
        int module_y = 0;
        for (Module module : MANAGER.MODULE.getModulesByCategory(current_category)) {
    ...
    public void loadConfigs() {
        ...
        (new Thread(() -> {
            int config_y = 3;
            for (String file1 : Objects.requireNonNull(MANAGER.CONFIG.getConfigList())) {
    ...
    public void renderGui(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        FontRenderers.modules.drawString(context.getMatrices(), "current cfg: " + MANAGER.CONFIG.currentConfig.getName(), main_posX + 120, main_posY + 18, new Color(0xCDFFFFFF, true).getRGB(), false);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int clickedButton) {
        ...
        if (isHoveringItem(main_posX + 368, main_posY + 17, 20, 6, (float) mouseX, (float) mouseY)) {
            if (listening_config) {
                MANAGER.CONFIG.save(config_string);
                config_string = "Save config";
            ...
            if (listening_friend) {
                MANAGER.FRIEND.addFriend(friend_string);
                friend_string = "Add friend";
    ...
    public void keyTyped(String typedChar, int keyCode) throws IOException {
        ...
        if (searching) {
            ...
            for (Module module : MANAGER.MODULE.getModulesSearch(search_string)) {
