package dev.thunderhack.gui.misc;

import dev.thunderhack.gui.font.FontRenderers;
import dev.thunderhack.modules.Module;
import dev.thunderhack.modules.misc.NoCommentExploit;
...
public class GuiScanner extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        radarx = Module.mc.getWindow().getScaledWidth() / 8;
        radarx1 = ((Module.mc.getWindow().getScaledWidth() * 5) / 8);
        radary = (Module.mc.getWindow().getScaledHeight() / 2) - ((radarx1 - radarx) / 2);
        radary1 = (Module.mc.getWindow().getScaledHeight() / 2) + ((radarx1 - radarx) / 2);

        centerx = (radarx + radarx1) / 2;
        centery = (radary + radary1) / 2;

        consolex = (int) ((Module.mc.getWindow().getScaledWidth() * 5.5f) / 8f);
        consolex1 = (Module.mc.getWindow().getScaledWidth() - 50);
        consoley = radary;
        ...
        Render2DEngine.drawRectDumbWay(context.getMatrices(), (float) ((Module.mc.player.getX() / 16 / 4f) + centerx), (float) ((Module.mc.player.getZ() / 16 / 4f) + centery), (float) (((Module.mc.player.getX() / 16 / 4f) + (radarx1 - radarx) / 300f) + centerx), (float) (((Module.mc.player.getZ() / 16 / 4f) + (radary1 - radary) / 300f) + centery), new Color(0x0012FF));
