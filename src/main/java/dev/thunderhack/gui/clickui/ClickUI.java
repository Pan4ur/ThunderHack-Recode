    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (!setup && ConfigManager.firstLaunch) {
            ...
            if (hstep == 0) {
                ...
                FontRenderers.sf_medium.drawCenteredString(context.getMatrices(), !MainSettings.isRu() ? "Hi! Thanks for using ThunderHack 1.20.1" : "Привет! Спс что скачал ThunderHack 1.20.1", hx, hy - 80, -1);
                FontRenderers.sf_medium.drawCenteredString(context.getMatrices(), !MainSettings.isRu() ? "Do you need help with the config?" : "Те помочь с кфг?", hx, hy - 70, -1);
                ...
                FontRenderers.sf_medium.drawCenteredString(context.getMatrices(), !MainSettings.isRu() ? "Choose your server" : "На какой сервер?", hx, hy - 80, -1);
