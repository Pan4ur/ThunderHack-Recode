package dev.thunderhack.setting;
...
public class Setting<T> {
    ...
    public float getPow2Value() {
        if (value instanceof Float) return (float) value * (float) value;
        if (value instanceof Integer) return (int) value * (int) value;
        return 0;
    }
    ...
    public boolean isVisible() {
        if (parent != null) {
            if (!parent.getValue().isExtended()) return false;
        }
