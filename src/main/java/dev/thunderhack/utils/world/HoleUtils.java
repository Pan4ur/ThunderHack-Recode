package dev.thunderhack.utils.world;
...
public final class HoleUtils {
    ...
    public static @NotNull List<BlockPos> getSurroundPoses(BlockPos from) {
        ...
        if (Module.mc.world == null) return surroundPoses;
    ...
    public static @Nullable List<BlockPos> findQuadPoses(BlockPos checkFrom) {
        if (Module.mc.world == null) return null;
        final List<BlockPos> quadPoses = new ArrayList<>();

        quadPoses.add(checkFrom);

        if (Module.mc.world.getBlockState(checkFrom.add(1, 0, 1)).isReplaceable()) {
            quadPoses.add(checkFrom.add(1, 0, 1));
            quadPoses.add(checkFrom.add(1, 0, 0));
            quadPoses.add(checkFrom.add(0, 0, 1));
        } else if (Module.mc.world.getBlockState(checkFrom.add(1, 0, -1)).isReplaceable()) {
            quadPoses.add(checkFrom.add(1, 0, -1));
            quadPoses.add(checkFrom.add(1, 0, 0));
            quadPoses.add(checkFrom.add(0, 0, -1));
        } else if (Module.mc.world.getBlockState(checkFrom.add(-1, 0, 1)).isReplaceable()) {
    ...
    private static boolean isIndestructible(BlockPos bp) {
        ...
        return Module.mc.world.getBlockState(bp).getBlock() == Blocks.OBSIDIAN
                || Module.mc.world.getBlockState(bp).getBlock() == Blocks.NETHERITE_BLOCK
                || Module.mc.world.getBlockState(bp).getBlock() == Blocks.CRYING_OBSIDIAN
                || Module.mc.world.getBlockState(bp).getBlock() == Blocks.RESPAWN_ANCHOR;
    }

    private static boolean isBedrock(BlockPos bp) {
        if (Module.mc.world == null) return false;

        return Module.mc.world.getBlockState(bp).getBlock() == Blocks.BEDROCK;
    }

    private static boolean isReplaceable(BlockPos bp) {
        if (Module.mc.world == null) return false;

        return Module.mc.world.getBlockState(bp).isReplaceable();
    }
}
