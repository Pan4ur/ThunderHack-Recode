...
public final class InteractionUtility {
    ...
    public static boolean canSee(Vec3d entityEyes, Vec3d entityPos) {
        if (Module.mc.player == null || Module.mc.world == null) return false;

        Vec3d playerEyes = getEyesPos(Module.mc.player);
        if (Module.mc.world.raycast(new RaycastContext(playerEyes, entityEyes, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, Module.mc.player)).getType() == HitResult.Type.MISS)
            return true;

        if (playerEyes.getY() > entityPos.getY())
            return Module.mc.world.raycast(new RaycastContext(playerEyes, entityPos, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, Module.mc.player)).getType() == HitResult.Type.MISS;
        return false;
    }
    ...
    public static boolean placeBlock(BlockPos bp, boolean rotate, Interact interact, PlaceMode mode, boolean ignoreEntities) {
        ...
        if (sprint)
            Module.mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(Module.mc.player, ClientCommandC2SPacket.Mode.STOP_SPRINTING));
        if (sneak)
            Module.mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(Module.mc.player, ClientCommandC2SPacket.Mode.PRESS_SHIFT_KEY));
        float[] angle = calculateAngle(result.getPos());
        if (rotate)
            Module.mc.player.networkHandler.sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(angle[0], angle[1], Module.mc.player.isOnGround()));
        ...
        if (mode == PlaceMode.Packet)
            Module.mc.player.networkHandler.sendPacket(new PlayerInteractBlockC2SPacket(Hand.MAIN_HAND, result, PlayerUtility.getWorldActionId(Module.mc.world)));

        awaiting.put(bp, System.currentTimeMillis());

        if (sneak)
            Module.mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(Module.mc.player, ClientCommandC2SPacket.Mode.RELEASE_SHIFT_KEY));
        if (sprint)
            Module.mc.player.networkHandler.sendPacket(new ClientCommandC2SPacket(Module.mc.player, ClientCommandC2SPacket.Mode.START_SPRINTING));
    ...
    public static BlockHitResult getPlaceResult(@NotNull BlockPos bp, Interact interact, boolean ignoreEntities) {
        if (!ignoreEntities)
            for (Entity entity : new ArrayList<>(Module.mc.world.getNonSpectatingEntities(Entity.class, new Box(bp))))
    ...
    public static List<Direction> getStrictDirections(BlockPos bp) {
        ...
        double westDelta = getEyesPos(Module.mc.player).x - (positionVector.add(0.5, 0, 0).x);
        double eastDelta = getEyesPos(Module.mc.player).x - (positionVector.add(-0.5, 0, 0).x);
        double northDelta = getEyesPos(Module.mc.player).z - (positionVector.add(0, 0, 0.5).z);
        double southDelta = getEyesPos(Module.mc.player).z - (positionVector.add(0, 0, -0.5).z);
        double upDelta = getEyesPos(Module.mc.player).y - (positionVector.add(0, 0.5, 0).y);
        double downDelta = getEyesPos(Module.mc.player).y - (positionVector.add(0, -0.5, 0).y);

        if (westDelta > 0 && !Module.mc.world.getBlockState(bp.west()).isReplaceable()) visibleSides.add(Direction.EAST);
        if (westDelta < 0 && !Module.mc.world.getBlockState(bp.east()).isReplaceable()) visibleSides.add(Direction.WEST);
        if (eastDelta < 0 && !Module.mc.world.getBlockState(bp.east()).isReplaceable()) visibleSides.add(Direction.WEST);
        if (eastDelta > 0 && !Module.mc.world.getBlockState(bp.west()).isReplaceable()) visibleSides.add(Direction.EAST);

        if (northDelta > 0 && !Module.mc.world.getBlockState(bp.north()).isReplaceable()) visibleSides.add(Direction.SOUTH);
        if (northDelta < 0 && !Module.mc.world.getBlockState(bp.south()).isReplaceable()) visibleSides.add(Direction.NORTH);
        if (southDelta < 0 && !Module.mc.world.getBlockState(bp.south()).isReplaceable()) visibleSides.add(Direction.NORTH);
        if (southDelta > 0 && !Module.mc.world.getBlockState(bp.north()).isReplaceable()) visibleSides.add(Direction.SOUTH);

        if (upDelta > 0 && !Module.mc.world.getBlockState(bp.down()).isReplaceable()) visibleSides.add(Direction.UP);
        if (upDelta < 0 && !Module.mc.world.getBlockState(bp.up()).isReplaceable()) visibleSides.add(Direction.DOWN);
        if (downDelta < 0 && !Module.mc.world.getBlockState(bp.up()).isReplaceable()) visibleSides.add(Direction.DOWN);
        if (downDelta > 0 && !Module.mc.world.getBlockState(bp.down()).isReplaceable()) visibleSides.add(Direction.UP);
            ...
            for (float x = 0f; x <= 1f; x += 0.2f) {
                for (float y = 0f; y <= 1; y += 0.2f) {
                    for (float z = 0f; z <= 1; z += 0.2f) {
                        ...
                        BlockHitResult wallCheck = Module.mc.world.raycast(new RaycastContext(InteractionUtility.getEyesPos(Module.mc.player), point, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, Module.mc.player));
                        if (wallCheck != null && wallCheck.getType() == HitResult.Type.BLOCK && wallCheck.getBlockPos() != bp)
                            continue;
                        BlockHitResult result = ExplosionUtility.rayCastBlock(new RaycastContext(InteractionUtility.getEyesPos(Module.mc.player), point, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, Module.mc.player), bp);
