...
public class Tracker extends Module {
    ...
    public void onUpdate() {
        ...
        for (PlayerEntity player : mc.world.getPlayers()) {
            ...
            if (found && only1v1.getValue()) {
                disable(MainSettings.isRu() ? "Ты не в дуели! Отключаю.." : "Disabled, you are not in a 1v1! Disabling...");
                return;
            }
            if (trackedPlayer == null)
                sendMessage(Formatting.LIGHT_PURPLE + (MainSettings.isRu() ? "Следим за " : "Now tracking ") + Formatting.DARK_PURPLE + player.getName().getString() + Formatting.LIGHT_PURPLE + "!");
        ...
        if (expStacks != exp) {
            expStacks = exp;
            if (MainSettings.isRu()) sendMessage(Formatting.DARK_PURPLE + trackedPlayer.getName().getString() + Formatting.LIGHT_PURPLE + " использовал " + Formatting.WHITE + exp + Formatting.LIGHT_PURPLE + (exp == 1 ? " стак" : " стаков") + " Пузырьков опыта!");
        ...
        if (crystalStacks != crystals) {
            ...
            if (MainSettings.isRu()) sendMessage(Formatting.DARK_PURPLE + trackedPlayer.getName().getString() + Formatting.LIGHT_PURPLE + " used " + Formatting.WHITE + crystals + Formatting.LIGHT_PURPLE + (crystals == 1 ? " stack" : " stacks") + " of Crystals!");
    ...
    public void sendTrack() {
        if (trackedPlayer != null) {
            ...
            if (MainSettings.isRu()) builder = new StringBuilder().append(trackedPlayer.getName().getString()).append(Formatting.LIGHT_PURPLE).append(" использовал ").append(Formatting.WHITE).append(c).append(Formatting.LIGHT_PURPLE).append(" (").append(Formatting.WHITE);
            ...
            if (MainSettings.isRu()) builder.append(Formatting.LIGHT_PURPLE).append(") кристаллов и ").append(Formatting.WHITE).append(e).append(Formatting.LIGHT_PURPLE).append(" (").append(Formatting.WHITE);
            ...
            if (MainSettings.isRu()) builder.append(Formatting.LIGHT_PURPLE).append(") пузырьков опыта.");
