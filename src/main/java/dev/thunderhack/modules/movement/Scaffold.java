...
public class Scaffold extends Module {
    ...
    private final dev.thunderhack.utils.Timer timer = new Timer();
    ...
    private InteractionUtility.BlockPosWithFacing checkNearBlocksExtended(BlockPos blockPos) {
        ...
        ret = InteractionUtility.checkNearBlocks(blockPos2.add(1, 0, 0));
        if (ret != null) return ret;

        ret = InteractionUtility.checkNearBlocks(blockPos2.add(-1, 0, 0));
        if (ret != null) return ret;

        ret = InteractionUtility.checkNearBlocks(blockPos2.add(0, 0, 1));
        if (ret != null) return ret;

        return InteractionUtility.checkNearBlocks(blockPos2.add(0, 0, -1));
    }
