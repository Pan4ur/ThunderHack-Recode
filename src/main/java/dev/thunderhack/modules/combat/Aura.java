...
public class Aura extends Module {
    ...
    public void handleKill() {
        if (target instanceof LivingEntity && (((LivingEntity) target).getHealth() <= 0 || ((LivingEntity) target).isDead()))
            ThunderHack.notificationManager.publicity("Aura", MainSettings.isRu() ? "Цель успешно нейтрализована!" : "Target successfully neutralized!", 3, Notification.Type.SUCCESS);
    }
    ...
    public Vec3d getLegitLook(Entity target) {
        ...
        if (!mc.player.canSee(target) && wallsBypass.getValue())
            return target.getPos().add(MathUtility.random(-0.15, 0.15), lenghtY, MathUtility.random(-0.15, 0.15));
