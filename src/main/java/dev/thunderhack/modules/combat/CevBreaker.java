...
import dev.thunderhack.mixins.accesors.IWorldRenderer;
...
public class CevBreaker extends Module {
    ...
    public void onSync(@SuppressWarnings("unused") EventSync e) {
        ...
        if (trap.getValue()) {
            ...
            for (BlockPos surroundBlock : HoleUtils.getSurroundPoses(target.getBlockPos()).stream().map(BlockPos::up).toList()) {
                if (InteractionUtility.canPlaceBlock(surroundBlock, interact.getValue(), false)) {
                    trapBlocks.add(surroundBlock);
                }
            }
            for (BlockPos headPos : HoleUtils.getHolePoses(target.getBlockPos()).stream().map(pos -> pos.up(2)).filter(pos -> !pos.equals(currentMineBlockPos)).toList()) {
    ...
    private void findTarget() {
        for (PlayerEntity player : ThunderHack.asyncManager.getAsyncPlayers()) {
            ...
            if (!HoleUtils.isHole(player.getBlockPos())) continue;
