...
public class HoleFill extends Module {
    ...
    public static final dev.thunderhack.utils.Timer inactivityTimer = new Timer();
    ...
    public void onSync(EventSync event) {
        ...
        if (HoleUtils.isHole(mc.player.getBlockPos()) && mc.world.getBlockState(mc.player.getBlockPos()).isAir()) {
            burrowWasEnabled = false;
        }
        ...
        while (blocksPlaced < actionShift.getValue()) {
            ...
            if (pos != null) {
                List<BlockPos> poses = HoleUtils.getHolePoses(pos).stream()
                        .filter(blockPos -> mc.player.getPos().distanceTo(blockPos.toCenterPos()) <= placeRange.getValue())
                        .toList();
                boolean broke = false;

                if (selfFillNeed && HoleUtils.isHole(mc.player.getBlockPos())) {
                    switch (selfFillMode.getValue()) {
                        ...
                        case Trap -> {
                            ...
                            if (mc.world.getBlockState(headPos).isReplaceable() && InteractionUtility.canPlaceBlock(headPos, interactMode.getValue(), false)) {
                                ...
                                for (int i = 0; i < 3; i++) {
                                    for (Vec3i vecAdd : HoleUtils.VECTOR_PATTERN) {
    ...
    private boolean isHole(BlockPos pos) {
        return ((HoleUtils.validTwoBlockIndestructibleXZ(pos) || HoleUtils.validTwoBlockBedrockXZ(pos)) && fillDouble.getValue())
                || ((HoleUtils.validQuadBedrock(pos) || HoleUtils.validQuadIndestructible(pos)) && fillQuad.getValue())
                || ((HoleUtils.validBedrock(pos) || HoleUtils.validIndestructible(pos)) && fillSingle.getValue());
    }
}