...
public class AutoMine extends Module {
    ...
    public void onSync(EventSync event) {
        ...
        if (!HoleUtils.isHole(targetPos)) return;

        // 1 прогон - ищем на которые можно поставить
        if (antiSurround.getValue()) {
            for (BlockPos offset : HoleUtils.getSurroundPoses(targetPos)) {
        ...
        if (!found && antiCiv.getValue()) {
            for (BlockPos pos : HoleUtils.getSurroundPoses(targetPos).stream()
                    .map(BlockPos::up)
                    .toList()) {
        ...
        if (!found && !oldVers.getValue() && antiSurround.getValue()) {
            for (BlockPos pos2 : HoleUtils.getSurroundPoses(targetPos)) {
